// Archivos y librerías necesarias para el funcionamiento del proyecto
...
#define DATA_PIN 27 //Pin utilizado en la placa (En este caso ESP32)
#define WIDTH 10 //Ancho de la matriz
#define HEIGHT 10 //Altura de la matriz
#define NUM_LEDS WIDTH * HEIGHT //Tamaño total de la matriz
#define BRIGHTNESS 240 //Brillo o intensidad de la matriz, recordando que el máximo es 255
#define FRAMES_PER_SECOND  120 //Dato necesario para el método doMultipleAnimations. Mientras más bajo el número, más lenta la animación.
...
// Cambiado de WS2812B a WS2811 ya que esta es la tira LED utilizada para el proyecto
...
//Termina el bloque encargado de trabajar el codigo para tetris

// Empieza el bloque de código para el uso de RFID
#define SS_PIN 5
#define RST_PIN 22
...
//Codigo set Multiplexores
  ...
  for (int i = 0; i < 7; i++) {
    ...
    //Set up Bluetooth
    Serial.begin(115200);
    SPI.begin();  // Iniciar SPI sin especificar pines (Si al compilar sale un error relacionado, no tiene importancia)
...
void selectFunction(String data) {
  if (data == "i") {
    while (true) {
      if (checkForBluetoothInterrupt()) { // Verifica si hay interrupción
        break; // Sale del bucle si hay datos disponibles en Bluetooth
      }
      doSpell("TableTop", -55); // Muestra el texto "TableTop"
    }
    ...
    } else if (data == "m") { // Lee sensores y controla LEDs
    while (true) {
      if (checkForBluetoothInterrupt()) { // Se verifica si hay interrupción
        break; // Sale del bucle si hay datos disponibles en Bluetooth
      }
...
void doMultipleAnimations(){
  gCurrentPatternNumber = 0; // resets the pattern number to 0 every time doMultipleAnimations is called
  ...
  if (SerialBT.available())
  {
    ...
    switch (keyPress)
    {
      ...
      case 'd':
        ...
        break;
      case 'x': //Exit Game
        FastLED.addLeds<WS2811, DATA_PIN, GRB>(leds, NUM_LEDS); // De nuevo, se cambia la configuración para que funcione con la tira WS2811
        matrix->begin(); //Sets up matrix
        matrix->setTextWrap(false);
        matrix->setBrightness(BRIGHTNESS);
        matrix->setTextColor(colors[pass]); // Inicializa con el primer color
