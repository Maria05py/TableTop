...
void start() {
    for (uint16_t time = 0, cycles = 0; cycles < 2608; time += 128, cycles++) {
        if (checkForBluetoothInterrupt()) { // Verifica si hay interrupción de Bluetooth o tarjeta
           return; // Salir de la función si hay alguna interrupción
        }
        
        calcFrame(time); // Calcula el frame de la animación
    }
    LEDS.clear(); // Limpia los LEDs después de la animación
}
